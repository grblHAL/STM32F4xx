...
If PlatformIO is used to build the driver the community provided and maintained [platformio.ini](https://github.com/grblHAL/STM32F4xx/blob/master/platformio.ini) can be used.
> [!NOTE]
> The _.ini_ file generated by the Web Builder can be used as a reference when modifying _platformio.ini_ when the provided file is not up to date. See [this link](https://github.com/grblHAL/core/wiki/Compiling-grblHAL#platformio).

To use install [PlatformIO Core](https://docs.platformio.org/en/latest/core/installation.html) and then:
...
> [!NOTE]
> As of Sep, 2021, the PlatformIO environments for the BigTreeTech SKR boards remain untested and might not work.  Feel free to drop a note of your success or failure with these boards on https://github.com/grblHAL/STM32F4xx/issues.
...
> [!NOTE]
> A build configuration for a debug version has not been added as a programmer/debugger is required and flashing a "normal" debug build is usually appropriate when debugging.
